...
void parse_joints(const sensor_msgs::JointState& arm){
  ...
  arm_steps.base_roll = (int)((arm.position[0]-prev_angle[0])*stepsPerRevolution[0]/(2*M_PI));
  arm_steps.shoulder_pitch = (int)((arm.position[1]-prev_angle[1])*stepsPerRevolution[1]/(2*M_PI));
  arm_steps.elbow_pitch = (int)((arm.position[2]-prev_angle[2])*stepsPerRevolution[2]/(2*M_PI));
  arm_steps.elbow_roll = (int)((arm.position[3]-prev_angle[3])*stepsPerRevolution[3]/(2*M_PI));
  ...
  total.base_roll += arm_steps.base_roll;
  total.shoulder_pitch += arm_steps.shoulder_pitch;
  total.elbow_pitch += arm_steps.elbow_pitch;
  total.elbow_roll += arm_steps.elbow_roll;
